...
> **最终比赛 Demo 不使用摇杆。** 实体输入只有一个 Button-PullUp 按钮，实体输出只有一个 LED1。仓库中出现的“摇杆 GPIO2 / WS2812 GPIO48”内容属于早期联调记录，不是当前接线、运行或验收步骤。
...
不需要连接摇杆、板载 RGB、LCD、麦克风、PIR、MicroSD 或其他传感器。
