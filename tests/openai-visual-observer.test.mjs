...
test("OpenAI visual observer sends one low-detail image and parses strict observation JSON", async () => {
  ...
  const visual = {
    ...
    evidence: ["正在编辑与任务目标一致的 RE:FOCUS 幻灯片"],
  };
  ...
  assert.deepEqual(result.observation, {
    ...
    progress_signals: visual.progress_signals,
    distraction_signals: visual.distraction_signals,
    confidence: visual.confidence,
  });
  assert.equal(result.relevance.classification, "relevant");
